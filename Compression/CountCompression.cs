...
namespace STS.General.Compression
{
    public static class CountCompression
    {
        ...
        private static int InternalGetSize(ulong number)
        {
            int index = 0;
        ...
        /// <summary>
        /// Returns value store size.
        /// </summary>
        /// <param name="count">Value for compression.</param>
        public static int GetSize(ulong number)
        {
            if (number < CACHE_SIZE)
                return cache[number].Length;
            else
                return InternalGetSize(number);
        }
