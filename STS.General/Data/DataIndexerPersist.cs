...
namespace STS.General.Data
{
    public class DataIndexerPersist : IIndexerPersist<IData>
    {
        ...
        //            ((Int64IndexerPersist)Persists[4]).Store(new BinaryWriter(streams[4]), (idx) => ((Data<Tick>)values.Invoke(idx)).Value.Volume, count);
        //        };

        //        actions[5] = () =>
        //        {
        //            streams[5] = new MemoryStream();
        //            ((StringIndexerPersist)Persists[5]).Store(new BinaryWriter(streams[5]), (idx) => ((Data<Tick>)values.Invoke(idx)).Value.Provider, count);
        //        };
        ...
        //                ((IIndexerPersist<Int64>)Persists[4]).Load(new BinaryReader(ms), (idx, value) => { ((Data<Tick>)array[idx]).Value.Volume = value; }, count);
        //        };

        //        actions[5] = () =>
        //        {
        //            using (MemoryStream ms = new MemoryStream(buffers[5]))
        //                ((IIndexerPersist<String>)Persists[5]).Load(new BinaryReader(ms), (idx, value) => { ((Data<Tick>)array[idx]).Value.Provider = value; }, count);
        //        };
